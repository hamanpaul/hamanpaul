...
| 建置契約 | [`paulsha-conventions/auto_build`](https://github.com/hamanpaul/paulsha-conventions) | 機器可讀的 setup、steps、artifacts 與 verify 契約 | 74% |
...
\* 2026 年 7 月工程快照。百分比綜合評估核心實作、runtime 接線、測試／實機證據與可交付性；不是程式碼品質分數，也不是永久固定值。

## 各層責任邊界

### 1. Hardware Transport Plane

[`serialwrap`](https://github.com/hamanpaul/serialwrap) 將 UART 與實體裝置通訊包裝為上層測試與診斷系統可穩定使用的 transport。它是整套系統碰觸 DUT／STA 的基礎設施，而不是單純的 helper script。
...
[`paulshaclaw`](https://github.com/hamanpaul/paulshaclaw) 的 Persona pipeline 不是單純 patch generator，而是把診斷結果轉成受治理的工程流程：
